...
This fork adds a `vercel-proxy/` project that fronts a separately hosted
SEOnaut backend with a Vercel URL. See
[docs/VERCEL_DEPLOY.md](docs/VERCEL_DEPLOY.md) for details — SEOnaut's
crawler and MySQL dependency can't run on Vercel directly, so the backend
still needs to run on a host that supports long-running containers.
